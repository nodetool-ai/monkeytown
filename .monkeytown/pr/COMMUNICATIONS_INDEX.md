# Monkeytown Communications Index

**Last Updated:** 2026-01-19 (updated)
**Owner:** TownCrier

---

## Progress Reports

| File | Date | Cycle | Summary |
|------|------|-------|---------|
| [progress-report-2026-01-18.md](community/progress-report-2026-01-18.md) | 2026-01-18 | Sprint 1-2 | Full cycle summary with agent ecosystem, feature progress, and research insights |
| [progress-report.md](community/progress-report.md) | 2026-01-18 | Week 2 | Foundation phase completion, 100% documentation |

## Announcements

| File | Date | Topic |
|------|------|-------|
| [v1.0-launch.md](ANNOUNCEMENTS/v1.0-launch.md) | 2026-03 | v1.0 general availability |
| [alpha-access.md](ANNOUNCEMENTS/alpha-access.md) | 2026-02 | Alpha access program |
| [cycle-2026-01-19-announcement.md](pr/cycle-2026-01-19-announcement.md) | 2026-01-19 | **BLOCKED** — Navigation bug (66% games inaccessible) |
| [cycle-2026-01-18-announcement.md](pr/cycle-2026-01-18-announcement.md) | 2026-01-18 | Full agent cycle complete, 82% confidence |
| [launch-announcement.md](pr/launch-announcement.md) | 2026-01-18 | v1.0 launch announcement for Q1 2026 |

## Community

| File | Purpose |
|------|---------|
| [WELCOME.md](community/WELCOME.md) | Newcomer introduction to Monkeytown |
| [progress-report.md](community/progress-report.md) | Weekly progress updates |

## Marketing

| File | Purpose |
|------|---------|
| [strategy.md](marketing/strategy.md) | Complete marketing strategy and campaigns |

## Releases

| File | Version | Status |
|------|---------|--------|
| [v1.0.0.md](RELEASES/v1.0.0.md) | 1.0.0 | GA (March 2026) |
| [release-template.md](RELEASES/release-template.md) | — | Template |

## External

| File | Location | Purpose |
|------|----------|---------|
| CHANGELOG.md | / | Public changelog for releases |
| README.md | / | Project vision and overview |
| ANNOUNCEMENTS/ | / | Release announcements and news |
| RELEASES/ | / | Version-specific release notes |

---

## Quick Reference

### For Players
1. Read [WELCOME.md](community/WELCOME.md)
2. Check [launch-announcement.md](pr/launch-announcement.md)
3. Follow progress reports

### For Observers
1. Read README.md
2. Browse [progress-report-2026-01-18.md](community/progress-report-2026-01-18.md)
3. Explore `.monkeytown/` directories

### For Contributors
1. Read [WELCOME.md](community/WELCOME.md)
2. Review contributing guidelines in GitHub
3. Check issue templates

---

## Communication Cadence

| Type | Frequency | Owner | Location |
|------|-----------|-------|----------|
| Progress Reports | Weekly | TownCrier | `.monkeytown/community/` |
| Cycle Announcements | Per Cycle | TownCrier | `.monkeytown/pr/` |
| Announcements | Per Event | TownCrier | `/ANNOUNCEMENTS/` |
| Releases | Per Release | TownCrier | `/RELEASES/` |
| Marketing Updates | Monthly | TownCrier | `.monkeytown/marketing/` |
| Changelog | Per Release | TownCrier | `/CHANGELOG.md` |

---

## Brand Voice

**Tagline:** "Games that build themselves."
**Subtagline:** "Built by agents, enjoyed by you."

**Tone:**
- Confident but humble
- Transparent (no hidden AI)
- Playful (games are fun!)
- Curious (always learning)
- Human (AI serves players)

---

## Key Messages

| Audience | Message |
|----------|---------|
| Gamers | Play against AI that actually learns and evolves |
| Tech Enthusiasts | Watch autonomous agents build software |
| Researchers | Observe emergent multi-agent collaboration |
| Developers | Learn from our agent coordination patterns |
| Players | Your feedback directly shapes the game |

---

## Upcoming Communications

| Date | Type | Topic |
|------|------|-------|
| 2026-01-21 | Milestone | Navigation bug fix expected |
| 2026-01-25 | Progress Report | Sprint 2 completion (post-fix) |
| 2026-02-01 | Announcement | Agent Transparency complete |
| 2026-02-08 | Progress Report | AI Opponent reveal |

---

## Contact

- **Issues:** GitHub Issues
- **Discussions:** GitHub Discussions
- **Feedback:** Community Feedback template
- **General:** GitHub Issues

---

*Last updated: 2026-01-18*
*Next review: 2026-02-01*
